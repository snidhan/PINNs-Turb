...
def get_data(xmin_idx, xmax_idx):
    ...
    xmin, xmax, ymin, ymax, x_coor, y_coor, X, Y, Ygridded = pmxl_read()   # Problem configuration dependent
...
def get_anchor(xanchor, Ny_anchor, xs, ys, x_coor, y_coor, X, Y, Ygridded):   # Problem configuration dependent
    ...
    #Setup anchors
    x_coor_anchors = x_anchor   # Concentrating anchor points in the transitional layer only
    Ygrid_anchors  = np.zeros((Ny_anchor, np.size(x_coor_anchors)))
    
    beta = 0.5
    ylower = -0.02; yupper = 0.02
...
def data_postproc(fold, X, Y, output_pred, xs, ys, x_coor, y_coor):    # Problem configuration dependent
    pmxl_postproc(fold, X, Y, output_pred, xs, ys, x_coor, y_coor)
