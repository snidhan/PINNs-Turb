...
def run_xde(xde_params, data_params, nn_params, fold_name, file_name, cudev=0):
    ...
    Re, xanchor, nym, bctype                   = data_params
    ...
    X, Y, Ygrid, xs, ys, xc, yc, y_coor_gridded = get_data()
    geom = dde.geometry.Rectangle([np.min(xc), np.min(yc)], [np.max(xc), np.max(yc)])

    if xanchor > 0:
        idx_anchors = get_anchor(xanchor, nym, xs, ys, xc, yc, X, Y, Ygrid, y_coor_gridded)
    ...
    def getvbc(n_bc,c):
            vbc = [[1.175/ys[0],0,0,0,0,0], [2.275/ys[0],0,0,0,0,0]]
    ...
    for i in range(nvar-1):
        #match bctype:
            ...
            if xanchor > 0:
                print('setting up the anchors', i)
                bc.append(dde.PointSetBC(X[idx_anchors], Y[idx_anchors,i:i+1], component=i))

                fig = plt.figure(figsize=(4,2.2), dpi=200)
                gsp = gridspec.GridSpec(1,1)
                ax0 = plt.subplot(gsp[0,0])
                cs0 = plt.plot(X[idx_anchors,1]*xs[1], Y[idx_anchors,i:i+1]*ys[i], 'rs', markersize=1)
                plt.grid()
                plt.savefig(str(i)  + 'anchors.png')
    ...
    #pp model
    data_postproc(fold_name, X, Y, model.predict(X), xs, ys, xc, yc, y_coor_gridded)
