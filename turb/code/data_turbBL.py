...
def turbBL_read(xmin_idx, xmax_idx):
    hf = h5py.File('../../data_central/zaki.h5', 'r')
...
def turbBL_postproc(fold, X, Y, output_pred, xs, ys, x_coor, y_coor):
    ...
    for l in range(0,axs.shape[0]):
        for m in range(0, axs.shape[1]):
            ...
            axs[l, m].plot(y_coor/theta[idx[count]], U[:,idx[count]], 'k-', linewidth=1.5)
            axs[l, m].plot(y_coor/theta[idx[count]], Ugrid[:,idx[count]], 'k--', linewidth=1.5)
            axs[l, m].set_title('U')
