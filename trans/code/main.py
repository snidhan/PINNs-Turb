...
glob_error = open('./unique_glob_error.txt', 'w')
...
for i,f in enumerate(fold_name):
    ...
    axs[0].semilogy(loss[:,0], np.sum(loss[:,1:4],1), '--', linewidth=1.5, label=str(uniq_inp[:,i])) # Training Residual Losses
    ...
    for j in range(nvar):
        ...
        if j != 2:
            if nxm[i] == 0:
                axs[c].semilogy(loss[:,0], np.sum(loss[:,idx_plot],1)/lossw[i], '--', linewidth=1.5, label=str(uniq_inp[:,i])) # BC losses if anchors ! given
            elif nxm[i] > 0:
                axs[c].semilogy(loss[:,0], loss[:,idx_plot[-1]]/lossw[i], '--', linewidth=1.5, label=str(uniq_inp[:,i])) # Anchor losses if anchors given
            c=c+1
glob_error.close()
